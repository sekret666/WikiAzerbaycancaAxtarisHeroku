...
app.post('/webhook', function (req, res) {
    ...
    if (req.body.result.parameters['Bored']) {
        ...
        callRecipePuppy(fooditem)
            .then((output) => {
                ...
                let displayText = `Tapdığım məlumat: ${output.title} at ${output.href}`;
                let telegramText = htmlEntities('*Found*-' + output.title + '\n' + '* Tərkibi aşağıdakılardır*-' + output.ingredients + '\n' + '* Siz onu yoxlaya bilərsiniz*- ' + output.href);
    ...
    else if (req.body.result.parameters['wikiaxtaristermini']) {
        var searchTerm = req.body.result.parameters['wikiaxtaristermini'];
        ...
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify({ 'speech': "Heçnə tapılmadı", 'displayText': "Heçnə tapılmadı" }));
    }
});
